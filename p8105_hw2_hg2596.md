...
In the beginning of cleaning this dataset it’s important to switch the
**Route** variable into character for the last 8-11 since 1-7 are also
character variables.
...
Converted the variable route from wide to long format, this helps us
know things about specific routes.In the following step we use distinct
function to help provide unique combinations.
...
In order to answer how many ADA complaint stations, we filter out for
ADA when true and use distinct to tell us how many are complaint.
...
In order to figure out which proportion of the station entrances and
exits without vending machines we filterd otu for vending machine when
no, and pulled out the column for enter to find the mean to be
`r trans_ent %>%filter(vending == "NO") %>% pull(entry) %>% mean`.
...
In order to get distinct variables, we needed to use pivot longer. Then
pull from our previous step to filter out the route for A and ada.
...
observations and 11 variables.The years included are 2001, 2012 years.
